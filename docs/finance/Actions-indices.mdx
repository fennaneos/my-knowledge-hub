...
<div className="tip-italic">
  <strong>💡 Interpretation:</strong><br/>
  The numerical yields in the boxes are not decorative — they represent the <em>discount rate applied to forward pricing</em>.<br/>
  Higher yield → lower forward → more negative forward premium → impacts <em>option skew</em> and <em>forward-start derivatives</em>.
</div>
...
<TryIt
  ...
  intro="Define the functions below, then run the tests. PASS/FAIL with numeric outputs appear on the Tests tab."
  starTotal={3}
  packs={[
    {
      id: 'one',
      name: '⭐ Define one-asset forward',
      detect: "def\\s+compute_forward\\s*\\(",
...
`,
    ...
    {
      id: 'idx',
      name: '⭐ Define index forward (vector)',
      detect: "def\\s+compute_index_forward\\s*\\(",
      scaffold: `import math
      ...
      tests: [
        {
          ...
          expected: (() => {
            const w=[0.5,0.3,0.2], S=[100,50,80], q=[0.03,0.02,0.04], r=0.02, T=1;
            let acc=0; for (let i=0;i<w.length;i++) acc += w[i]*S[i]*Math.exp((r-q[i])*T);
        ...
        },
        {
          ...
          expected: (() => {
            const w=[0.4,0.6], S=[120,80], q=[0.015,0.025], r=0.02, T=1.5;
            let acc=0; for (let i=0;i<w.length;i++) acc += w[i]*S[i]*Math.exp((r-q[i])*T);
            return Number(acc.toFixed(6));
          })(),
          tol: 1e-3,
